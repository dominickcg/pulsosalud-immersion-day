...
echo -e "${CYAN}Búsqueda con Embeddings del Día 2:${NC}"
echo -e "${GRAY}---${NC}"
echo -e "${GRAY}La parte clave es el operador de distancia coseno:${NC}"
echo -e "${GREEN}  <=> ${NC}${GRAY}(operador de pgvector para calcular similitud)${NC}\n"
echo -e "${GRAY}Ejemplo simplificado:${NC}"
echo -e "${GRAY}  SELECT informes WHERE embedding <=> [vector_referencia]${NC}"
echo -e "${GRAY}  ORDER BY similitud DESC${NC}"
echo -e "${GRAY}---\n${NC}"
echo -e "${YELLOW}Diferencia clave:${NC}"
echo -e "${GRAY}• SQL Día 1: WHERE trabajador_id = 1 (coincidencia exacta)${NC}"
echo -e "${GRAY}• Embeddings Día 2: WHERE embedding <=> vector (similitud semántica)${NC}\n"
